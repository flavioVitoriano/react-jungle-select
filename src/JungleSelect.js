...
class JungleSelect extends Component {
      ...
      if (Immutable.List.isList(items)) {
        ...
        groups.forEach((group) => {
          sortedItems = sortedItems.concat(
            items.filter((i) => i.get('groupId') === group.get('id'))
          );
        });
      } else {
        sortedItems = [];
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.focus !== prevProps.focus) {
      const element = this.filterInput
        ? this.filterInput
    ...
    if (
      ...
      if (element && container) {
        ...
        if (elementScrollHeight > containerHeight - elementHeight) {
          container.scrollTop =
            elementScrollHeight - containerHeight + elementHeight;
  ...
  groupFromItem(item) {
    ...
    let groupId = Immutable.Map.isMap(item)
      ? item.get('groupId')
      : item.groupId;
  ...
  highlightItemFromKeyboard(dir, e, selectOnHighlight) {
    e.preventDefault();
    ...
    if (highlighted != null) {
      switch (dir) {
        case 'next':
          ...
          if (selectOnHighlight) this.selectHighlightedItem(e);
          break;
        case 'prev':
          newHighlighted = highlighted - 1;
          if (newHighlighted < 0) {
  ...
  onClear(e) {
    ...
    let clearClicked =
      e && e.target && e.target.classList.contains('jungle-select-clear');
  ...
  filterMethod(string) {
    ...
    switch (filteringMode) {
      case 'any':
        return search
          .split(' ')
          .some((s) => sanitizeSearchString(string).indexOf(s) !== -1);
      case 'every':
        return search
          .split(' ')
          .every((s) => sanitizeSearchString(string).indexOf(s) !== -1);
      default:
  ...
  fetchItems = debounce((filter) => {
    const {
      remote: { baseUrl, itemsId, queryParams, searchParam },
    ...
    fetch(url)
      ...
      .then((response) => {
        ...
        this.setState({
          listOpened: !!filter && !!filter.length && !!additionalItem,
        });
      })
      .catch((ex) => {
        console.log(ex);
  ...
  highlightItems(items) {
    ...
    this.highlights = items.map((item, index) => {
      ...
      searchableAttributes.forEach((k) => {
        const key = Array.isArray(k) ? k : [k];
        matches = matches.setIn(
          key,
          this.highlightFilterMatches(item.getIn(key))
        );
      });
      return matches;
    });
      ...
      if (filterItem) {
        ...
        filtered = items.filter((item, index) => {
          if (typeof item === 'string') {
            ...
            if (match) {
              this.highlights = this.highlights.set(
                index,
            ...
            if (Immutable.Map.isMap(item)) {
              ...
              let matches = item;
              let matching = searchableAttributes
                .map((k) => {
                  let match = false;
                  if (Array.isArray(k)) {
                    ...
                    if (match) {
                      matches = matches.setIn(
                        k,
                        this.highlightFilterMatches(item.getIn(k))
                      );
                    ...
                    if (match) {
                      matches = matches.set(
                        k,
                        this.highlightFilterMatches(item.get(k))
                      );
                      this.highlights = this.highlights.set(index, matches);
                    }
                  }
              ...
              let matching = searchableAttributes
                .map((k) => {
                  ...
                  if (match) {
                    matches = matches.set(
                      k,
                      this.highlightFilterMatches(item[k])
                    );
                    this.highlights = this.highlights.set(index, matches);
                  }
                  return match;
  ...
  itemsForGroup(group) {
    ...
    if (Immutable.List.isList(sortedItems)) {
      return sortedItems.filter(
        (item) => item.get('groupId') === group.get('id')
      );
    } else {
  ...
  renderList() {
    ...
    if (groups) {
      return groups.map((group, groupIndex) => {
        ...
        return (
          <section key={groupIndex} className="jungle-select-group">
            <div className="jungle-select-group-title">
      ...
      const limitedSize = Immutable.List.isList(limited)
        ? limited.size
        : limited.length;
      const filteredSize = Immutable.List.isList(filtered)
        ? filtered.size
        : filtered.length;
  ...
  renderInternalItem(item, index) {
    ...
    return (
      <div
        ...
        onMouseLeave={
          highlighted === index
            ? this.highlightItem.bind(this, null, false)
            : null
        }
      >
  ...
  highlightFilterMatches(text) {
    const { filter } = this.state;
    let regexedFilter = removeDiacritics(filter)
      .trim()
      .replace(/[\s]+/g, '|')
      .split('')
      .map((l) => {
        ...
        return this.letterToDiacritics[l]
          ? `[${l + this.letterToDiacritics[l]}]`
          : l;
      })
      .join('');
    ...
    return (
      <span dangerouslySetInnerHTML={{ __html: text.replace(regex, subst) }} />
    );
  }
  ...
  toggleList(e) {
    ...
    this.selectMode() && this.state.listOpened && !searchable
      ? this.blur()
      : this.focus();
  ...
  renderSelectedItems(items) {
    ...
    return items.map((item, i) => (
      <div className="jungle-select-selected-value" key={i} title={item}>
  ...
  render() {
    ...
    const handlers = {
      up: ::this.highlightItemFromKeyboard.bind(
        this,
        'prev',
        this.props.selectOnHighlight
      ),
      down: ::this.highlightItemFromKeyboard.bind(
        this,
        'next',
        this.props.selectOnHighlight
      ),
        ...
        <div>
          <div className="jungle-select-controls">
            ...
            {(filter.length || (selected.length !== 0 && clearable)) && (
              <div className="jungle-select-clear" onClick={::this.onClear}>
                {this.clearNode()}
              </div>
            )}
          </div>
...
const arrayOrListOfGroups = PropTypes.oneOfType([
  ...
  ImmutablePropTypes.listOf(PropTypes.oneOfType([ImmutablePropTypes.map])),
]);
...
JungleSelect.propTypes = {
  ...
  searchableAttributes: PropTypes.arrayOf(PropTypes.string),
